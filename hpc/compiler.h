...
#define __init__priority__(p) __attribute__((constructor(p)))
...
#define STR_CONCAT_(prefix, suffix) prefix##suffix
#define STR_CONCAT(prefix, suffix) STR_CONCAT_(prefix, suffix)
#define MAKE_UNIQUE_VARIABLE_NAME(prefix) STR_CONCAT(prefix##_, __LINE__)

/* Define the STATIC_ASSERT macro */
#if (__GNUC__)
#define STATIC_ASSERT(condition, message) _Static_assert(condition, message)
#else
#error "This STATIC_ASSERT macro is only compatible with the GCC compiler."
#endif
...
#define BUILD_TIME_NUM_BITS(N) (1 + ((N) > 1) + ((N) > 3) + ((N) > 7) + \
